...
for(i in 1:24){
  ...
  result.1 <-
    foreach(a=chunk1)%dopar% {
      ...
      # replace predictions with NA, if original row contained NA
      data_out[index] <- NA
  ...
  writeRaster(template_ras, filename=paste("whcr_daytime_", i, "_.tif", sep=""),format="GTiff",datatype="FLT4S", overwrite=TRUE)
}
...
list2 <- list()
for(i in 1:24){ 
  rx <- raster(paste("whcr_daytime_", i, "_.tif", sep=""))
  list2[[i]] <- rx
}
...
writeRaster(rast.mosaic, filename = "within_HR_level_prediction_daytime.tif",
